...
| `POST` | `/api/internal/python-compute-shadow`    | Run a live, SAMPLED shadow comparison of `resolveReentryRisk()` (TS, authoritative) vs the Python compute engine against the current catalog, persist it, does not affect production risk scoring (cron-job.org, recommended every 15-30 min during live rollout; see docs/PYTHON_COMPUTE_SHADOW_ROLLOUT.md). Samples via a two-phase query (`lib/shadowCatalog.ts`'s `loadCurrentTrendSample`) rather than loading the full trend population — never pulls TIP data, which this route doesn't use. Response includes a `timing` breakdown (`catalogLoadMs`/`pythonComputeMs`/`persistenceMs`/`totalRouteMs`), also persisted on the run record. `?sampleRate=` (default 0.15), `?maxSampleSize=` (default 20, hard-capped at 25). `x-internal-secret` auth |
