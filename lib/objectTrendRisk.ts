...
import { getReentryRisk, parseBSTAR } from './satelliteHelpers';
...
export function objectTrendToReentryRisk(
  ...
  const decayRateKmPerDay = Math.max(
    trend.perigeeSlope14d && trend.perigeeSlope14d < 0
      ...
      : 0,
    trend.smaSlope14d && trend.smaSlope14d < 0
      ? Math.abs(trend.smaSlope14d)
      : 0
  );
  ...
  return {
    ...
    signalsAgree: debris ? trendSignalsAgree(trend) : allTrendSignalsAgree(trend),
